...
//	str1=str1 + 'value=' + arr[arr.length-1] +'>' +arr[arr.length-1]+'<br/>';
	...
	for(i=0;i<arr.length;i++)
	{
		strr+='<tr>'+'<td>'+arr[i].roll+'</td>'+'<td>'+arr[i].CGPA+'</td>'+'<td>'+'<input type='+'"checkbox"'+''+'name='+'"students_selected"'+'class='+'"messageCheckbox"';
...
var back1=function(){
	...
	$.post('http://127.0.0.1:5050/addStudent',{roll:$("#roll2").val(),
			...
			status1:'not at nominated',
			status2:'not at nominated',
			status3:'not at nominated'},function(data){
